...
- **变量面板（Watch）**：所有标号的地址 + **当前值**（十六进制 / 十进制 / 字符提示），**双击"值"列可在运行时直接修改内存**（支持 `0041`、`#41`、`65`、`-1`、`'A'` 等写法）
- **内存面板**：8×8 十六进制窗口，高亮 PR 指向的指令字，双击任意单元可直接改写
- **输入 / 输出面板**：`OUT` 输出显示于此；程序执行 `IN` 时解锁输入框等待一行输入
...
- `IN BUF,LNG` / `OUT BUF,LNG`：第二个操作数是存放长度的标号；缓冲区建议 `DS 256`
...
三层测试全部通过后发布：核心库 24 项（汇编器 + 机器）、运行器 12 项（信号投递、断点、单步）、GUI 43 项（离屏驱动真实主窗口：面板刷新、查找条、按钮门控、Watch 编辑）。
